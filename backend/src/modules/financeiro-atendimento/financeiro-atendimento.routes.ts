...
import { createFinanceiroAtendimentoService } from './financeiro-atendimento.service.js';
import { criarProntuarioSchema, atualizarProntuarioSchema, criarConsultaSchema } from './financeiro-atendimento.schemas.js';

const PROF = ['FINANCEIRO_ATENDIMENTO', 'DIRETORIA'] as const;

export default async function financeiroAtendimentoRoutes(app: FastifyInstance) {
  const service = createFinanceiroAtendimentoService(app);
